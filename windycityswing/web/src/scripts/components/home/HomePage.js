...
class HomePage extends React.Component {
  ...
  componentWillMount() {
    axios
      .get(
        `http://${window.location.hostname}:8080/dances?year=${this.state.currentYear}&month=${this.state.currentMonth}`
      )
      .then((response) => {
        this.setState({
  ...
  isAnException(event, day) {
    if (event.recurrence && event.recurrence.exceptions) {
      ...
      const exceptionString =
        this.state.currentMonthString.substring(
          0,
          this.state.currentMonthString.indexOf("-", 5)
        ) + dayString;
      const exceptionFinder = (exception) => {
        return exception === exceptionString;
      };
  ...
  renderCalendar() {
    const dayOfTheWeekOfTheFirstDayOfTheMonth = Number(
      moment(this.state.currentMonthString).format("d")
    );
    ...
    for (let i = 0; i < this.state.events.length; i++) {
      ...
      if (
        event.date.year === this.state.currentYear &&
        event.date.month === this.state.currentMonth &&
        (!event.recurrence ||
          !event.recurrence.rule ||
          event.recurrence.rule === "none")
      ) {
        eventMap[event.date.day].push(event);
      }
      if (event.recurrence) {
        if (event.recurrence.rule === "dayOfWeek") {
          let currentDayOfEvent =
            event.recurrence.dayOfWeek -
            dayOfTheWeekOfTheFirstDayOfTheMonth +
            1;
          if (
            currentDayOfEvent > 0 &&
            !this.isAnException(event, currentDayOfEvent)
          ) {
            eventMap[
              event.recurrence.dayOfWeek -
                dayOfTheWeekOfTheFirstDayOfTheMonth +
          ...
          currentDayOfEvent += 7;
          if (
            currentDayOfEvent <= maxDaysInCurrentMonth &&
            !this.isAnException(event, currentDayOfEvent)
          ) {
            eventMap[currentDayOfEvent].push(event);
          }
          ...
          event.recurrence.rule === "dayOfMonth" &&
          !this.isAnException(event, event.recurrence.dayOfMonth)
        ) {
      ...
      const startTimestamp = moment(
        event.timezone.startTimestamp,
        "YYYYMMDDTHHmmss"
      );
      const endTimestamp = moment(
        event.timezone.endTimestamp,
        "YYYYMMDDTHHmmss"
      );
      if (
        startTimestamp.format("YYYYMMDD") !== endTimestamp.format("YYYYMMDD") &&
        endTimestamp.from(startTimestamp).indexOf("days") !== -1 &&
        event.date.month === this.state.currentMonth
      ) {
        for (
          let i = 1;
          i <= Number(endTimestamp.from(startTimestamp).substr(3, 1));
          i++
        ) {
    ...
    const firstWeek = (
      <tr key={1} className="event-calendar__week-one">
        <CalendarDay
          events={
            dayOfTheWeekOfTheFirstDayOfTheMonth === 0
              ? eventMap[currentDay]
              : null
          }
        ...
        <CalendarDay
          events={
            dayOfTheWeekOfTheFirstDayOfTheMonth <= 1
              ? eventMap[currentDay]
              : null
          }
          onClick={this.openModal}
        ...
        <CalendarDay
          events={
            dayOfTheWeekOfTheFirstDayOfTheMonth <= 2
              ? eventMap[currentDay]
              : null
          }
          onClick={this.openModal}
        ...
        <CalendarDay
          events={
            dayOfTheWeekOfTheFirstDayOfTheMonth <= 3
              ? eventMap[currentDay]
              : null
          }
          onClick={this.openModal}
        ...
        <CalendarDay
          events={
            dayOfTheWeekOfTheFirstDayOfTheMonth <= 4
              ? eventMap[currentDay]
              : null
          }
          onClick={this.openModal}
        ...
        <CalendarDay
          events={
            dayOfTheWeekOfTheFirstDayOfTheMonth <= 5
              ? eventMap[currentDay]
              : null
          }
          onClick={this.openModal}
        ...
        <CalendarDay
          events={
            dayOfTheWeekOfTheFirstDayOfTheMonth <= 6
              ? eventMap[currentDay]
              : null
          }
          onClick={this.openModal}
    ...
    if (
      ...
      fifthWeek = (
        <tr key={5}>
          <CalendarDay
            events={
              currentDay <= maxDaysInCurrentMonth ? eventMap[currentDay] : null
            }
            onClick={this.openModal}
  ...
  openModal(day, events) {
    ...
    const weekday = moment(
      this.state.currentMonthString.replace("-01", dayString),
      "YYYY-MM-DD"
    ).day();
  ...
  goToPreviousMonth() {
    ...
    axios
      .get(
        `http://${window.location.hostname}:8080/dances?year=${previousYear}&month=${previousMonth}`
      )
      .then((response) => {
        this.setState({
          events: response.data,
    ...
    history.push(`/${previousYear.toString()}-${previousMonth.toString()}`);
  }
  ...
  goToNextMonth() {
    ...
    axios
      .get(
        `http://${window.location.hostname}:8080/dances?year=${nextYear}&month=${nextMonth}`
      )
      .then((response) => {
        this.setState({
          events: response.data,
    ...
    history.push(`/${nextYear.toString()}-${nextMonth.toString()}`);
  }
      ...
      <div className="windy-city-home">
        <table className="event-calendar">
          <tbody>
            <tr>
              <td
                className="event-calendar__navigator"
                colSpan="2"
                onClick={this.goToPreviousMonth}
              >
                {"< Previous"}
              </td>
              <td className="event-calendar__month" colSpan="3">
                {moment(this.state.currentMonthString).format("MMMM YYYY")}
              </td>
              <td
                className="event-calendar__navigator"
                colSpan="2"
        ...
        <span className="event-calendar__instructions">
          Select one of the days above to see the events happening on that day
        </span>
        <CalendarDayModal
