...
describe("event table", () => {
  ...
  it("has the first day of the month in the correct column of the first week in the calendar", () => {
    ...
    const dayOfTheWeekOfTheFirstDayOfTheMonth = moment(
      moment().format("YYYY") + "-" + moment().format("MM") + "-01"
    ).format("d");

    expect(
      firstWeek.children().at(dayOfTheWeekOfTheFirstDayOfTheMonth).props().day
    ).toEqual(1);
  });
      ...
      request
        ...
        .then(
          () => {
            ...
            expect(tree.instance().state.events[0].title).toEqual(
              "previous month's dances"
            );
            done();
          },
          () => {
  ...
  it("changes the url when going to the previous month", (done) => {
    ...
    moxios.wait(() => {
      ...
      request
        ...
        .then(
          () => {
            expect(mockPushHistory).toBeCalledWith(
              `/${year.toString()}-${previousMonth.toString()}`
            );
            done();
          },
          () => {
    ...
    moxios.wait(() => {
      ...
      request
        ...
        .then(
          () => {
            expect(mockPushHistory).toBeCalledWith(
              `/${year.toString()}-${nextMonth.toString()}`
            );
            done();
          },
          () => {
  ...
  describe("handling event placement", () => {
    ...
    it("sends an event to the correct day", () => {
      ...
      expect(theDayThatThisEventShouldBeOn.props().events).toContainEqual(
        testEvent
      );
    });
    ...
    it("should have an onClick handler", () => {
      ...
      expect(tree.find("CalendarDay").at(30).props().onClick).toEqual(
        tree.instance().openModal
      );
    });
  });
  ...
  describe("handing recurring events", () => {
    ...
    it("places a monthly event by day of month", () => {
      ...
      expect(tree.find("CalendarDay").at(2).props().events).toContainEqual(
        testEvent
      );
    });
    ...
    it("does not place an monthly event by day of month if it is an exception", () => {
      ...
      expect(tree.find("CalendarDay").at(2).props().events).not.toContainEqual(
        testEvent
      );
    });
    ...
    it("places a monthly event by day of week and week of month", () => {
      ...
      expect(tree.find("CalendarDay").at(16).props().events).toContainEqual(
        testEvent
      );
    });
    ...
    it("does not place an monthly event by day of week of month if it is an exception", () => {
      ...
      expect(tree.find("CalendarDay").at(16).props().events).not.toContainEqual(
        testEvent
      );
    });
    ...
    it("places an event on the last day of week of month", () => {
      ...
      expect(tree.find("CalendarDay").at(27).props().events).toContainEqual(
        testEvent
      );
    });
    ...
    it("places an event on multiple days if it is a multiple day event", () => {
      ...
      expect(tree.find("CalendarDay").at(2).props().events).toContain(
        testEvent
      );
      expect(tree.find("CalendarDay").at(3).props().events).toContain(
        testEvent
      );
      expect(tree.find("CalendarDay").at(4).props().events).toContain(
        testEvent
      );
    });
  });
...
describe("handling the modal", () => {
  ...
  it("should show the current month", () => {
    ...
    expect(tree.find("CalendarDayModal").props().month).toEqual(
      state.currentMonth
    );
  });
  ...
  it("can close the modal", () => {
    ...
    expect(
      tree.find("CalendarDayModal").props().closeModal
    ).not.toBeUndefined();
    expect(tree.find("CalendarDayModal").props().closeModal).toEqual(
      tree.instance().closeModal
    );
  });
});
