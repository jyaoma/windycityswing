...
it("should hide", () => {
  tree = shallow(
    <CalendarDayModal day={1} events={passedInEvents} hidden={true} />
  );
...
it("should render the same number of events as passed into the modal", () => {
  expect(tree.find(".calendar-day-modal__event").length).toEqual(
    passedInEvents.length
  );
});

it("can be closed", () => {
  expect(
    tree.find(".calendar-day-modal__close-container").props().onClick
  ).toEqual(closeModal);
});
...
describe("events", () => {
  it("leads to the event detail page", () => {
    ...
    expect(history.push).toBeCalledWith("/test-event--2017-05-01");
  });
});
